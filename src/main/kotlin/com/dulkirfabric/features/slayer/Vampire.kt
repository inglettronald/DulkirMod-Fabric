...
object Vampire {
    ...
    private val box = AABB(-.4, .1, -.4, .4, -1.9, .4)
    private var inT5 = false
    private val ichorBox = AABB(- .5, 1.0, -.5, .5, 2.0, .5,)
    ...
    fun steakDisplay(event: WorldRenderLastEvent) {
        ...
        ents.forEach {
            if (it !is ArmorStand || !it.hasCustomName()) return@forEach
            ...
            if (name.contains(char) && name.contains("Bloodfiend")) {
                val pos = it.getInterpolatedPos(mc.deltaTracker.getGameTimeDeltaPartialTick(true))
    ...
    fun ichorHighlight(event: WorldRenderLastEvent) {
        ...
        ents.forEach {
            ...
            val itemStack = it.getItemBySlot(EquipmentSlot.HEAD) ?: return@forEach
            ...
            WorldRenderUtils.drawWireFrame(
                event.context,
                ichorBox.move(pos.x, pos.y, pos.z),
                Color(0, 255, 255),
                8f,
            )
        }
